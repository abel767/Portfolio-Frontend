...
export function MobileContact() {
  ...
  return (
    <section ref={ref} id="contact" className="min-h-screen bg-background py-20 px-6">
      <div className="max-w-2xl mx-auto">
        ...
        >
          <div className="inline-block px-3 py-1 bg-accent/10 border border-accent/30 text-accent mb-6 font-mono">
            Contact
          </div>
        ...
        >
          {["name", "email", "message"].map((field) => {
            ...
            return (
              <div key={field}>
                <label htmlFor={field} className="block mb-2 text-foreground/90 capitalize">{field}</label>
                {isTextarea ? (
                  <textarea
                    id={field}
                    rows={5}
                    ...
                    className="w-full px-4 py-3 bg-card border border-border text-foreground placeholder-foreground/30 focus:outline-none transition-all duration-300 resize-none"
                    style={{ borderColor: focusedField === field ? "var(--accent)" : "var(--border)" }}
                  />
                ) : (
                  <input
                    id={field}
                    ...
                    onFocus={() => setFocusedField(field)}
                    onBlur={() => setFocusedField(null)}
                    className="w-full px-4 py-3 bg-card border border-border text-foreground placeholder-foreground/30 focus:outline-none transition-all duration-300"
                    style={{ borderColor: focusedField === field ? "var(--accent)" : "var(--border)" }}
                  />
                )}
              </div>
          ...
          <button
            type="submit"
            disabled={status.loading}
            className="w-full px-6 py-4 bg-accent text-foreground hover:bg-accent/90 transition-colors font-mono flex items-center justify-center gap-2"
          >
        ...
        >
          ...
          <div className="relative flex justify-center">
            <span className="px-4 bg-background text-muted-foreground font-mono">or reach out via</span>
          </div>
        </motion.div>
